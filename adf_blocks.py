...
class AvgPool2d(nn.Module):
    ...
    def __init__(self, keep_variance_fn=None,kernel_size=7):
        super(AvgPool2d, self).__init__()
        self._keep_variance_fn = keep_variance_fn
        self._kernel_size = kernel_size
    def forward(self, inputs_mean, inputs_variance, kernel_size=None):
        if kernel_size==None:
...
class ReLU(nn.Module):
    ...
    def forward(self, features_mean, features_variance):
        ...
        pdf = utils.normpdf(div)
        cdf = utils.normcdf(div)
...
class MaxPool2d(nn.Module):
    ...
    def _max_pool_internal(self, mu_a, mu_b, var_a, var_b):
        ...
        ab = mu_a - mu_b
        alpha = ab / stddev
        pdf = utils.normpdf(alpha)
        cdf = utils.normcdf(alpha)
...
class Sequential(nn.Module):
    ...
    def forward(self, input_mean, inputs_variance):
        for module in self._modules.values():
            input_mean, inputs_variance = module(input_mean, inputs_variance)
