...
def make_layers(cfg: List[Union[str, int]],dropout_prob,min_variance: float =1e-5,input_channel: int= 3) -> adf_blocks.Sequential:
    ...
    for v in cfg:
        if v == 'M':
            ...
            layers += [conv2d, adf_blocks.BatchNorm2d(v,keep_variance_fn=var_fun), adf_blocks.LeakyReLU(keep_variance_fn=var_fun)]
            in_channels = v
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, p=0.2, keep_variance_fn=None):
        ...
        self.ReLU = adf_blocks.LeakyReLU(keep_variance_fn=self.keep_variance_fn)
...
class ResNetADF(nn.Module):
    def __init__(self, block, num_blocks,input_channel=3, num_classes=10, p=0.1, noise_variance=1e-3, min_variance=1e-3):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, p=0.1, keep_variance_fn=None):
...
class AllConvNetADF(nn.Module):
    def __init__(self,input_channel,num_classes,dropout_prob,noise_variance=1e-3, min_variance=1e-3, log_variance=False):
        super(AllConvNetADF, self).__init__()
        self._keep_variance_fn = lambda x: keep_variance(x, min_variance=min_variance)
        self._noise_variance = noise_variance
        self.num_classes=num_classes

        self.conv1   = make_conv(input_channel, 96, kernel_size=3, stride=1, keep_variance_fn=self._keep_variance_fn)
        ...
        self.dropout1 = adf_blocks.Dropout(dropout_prob, keep_variance_fn=self._keep_variance_fn)
        ...
        self.dropout2 = adf_blocks.Dropout(dropout_prob, keep_variance_fn=self._keep_variance_fn)
        ...
        self.conv3_2 = make_conv(192,  self.num_classes, kernel_size=1, stride=1, nonlinear=False, keep_variance_fn=self._keep_variance_fn)


    def forward(self, inputs_mean):
        inputs_variance = torch.zeros_like(inputs_mean) + self._noise_variance
        x = inputs_mean, inputs_variance
        ...
        mu = mu.contiguous().view(batch_size, self.num_classes, -1)
        variance = variance.contiguous().view(batch_size, self.num_classes, -1)
...
def all_conv_net(input_channel:int=3, dropout_prob:float=0.2 , **kwargs: Any)  -> AllConvNetADF :
    return AllConvNetADF(input_channel=input_channel,num_classes=kwargs['num_classes'],dropout_prob=dropout_prob,min_variance=kwargs['min_variance'],noise_variance=kwargs['noise_variance'])    




def vgg19(input_channel:int=3, dropout_prob:float=0.2 ,**kwargs: Any) -> VGGADF:
    return VGGADF(make_layers(cfgs['E'],input_channel=input_channel,dropout_prob=dropout_prob), drop_probability=dropout_prob,**kwargs)
def vgg16(input_channel:int=3, dropout_prob:float=0.2 ,**kwargs: Any) -> VGGADF: 
    return VGGADF(make_layers(cfgs['D'],input_channel=input_channel,dropout_prob=dropout_prob),drop_probability=dropout_prob, **kwargs)
def vgg13(input_channel:int=3, dropout_prob:float=0.2 ,**kwargs: Any) -> VGGADF:
    return VGGADF(make_layers(cfgs['B'],input_channel=input_channel,dropout_prob=dropout_prob),drop_probability=dropout_prob, **kwargs)
def vgg11(input_channel:int=3, dropout_prob:float=0.2 ,**kwargs: Any) -> VGGADF:
    return VGGADF(make_layers(cfgs['A'],input_channel=input_channel,dropout_prob=dropout_prob),drop_probability=dropout_prob, **kwargs)
